...
\begin{align}
\partial_t y - \nu \Delta y + f(y) &= 0, \quad &&\text{in } Q, \label{eq:state_res}\\
-\partial_t \lambda - \nu \Delta \lambda + f'(y) \lambda &= -(y - y_d), \quad &&\text{in } Q, \label{eq:adjoint_res}\\
\alpha u - \partial_n \lambda &= 0, \quad &&\text{on } \Sigma, \label{eq:stat_res}\\
...
Let $Y_\theta$, $U_\psi$, $\Lambda_\phi$ be the neural network approximations of $y$, $u$, $\lambda$ built with the hard-constraint construction of Section~2.2.3, so that the Dirichlet conditions \eqref{eq:bc_y_res}--\eqref{eq:bc_lambda_res} hold \emph{exactly, for every parameter value}. We define the residuals of the optimality system:

\begin{align}
\mathcal{R}_y(x,t;\theta,\psi) &:= \partial_t Y_\theta - \nu \Delta Y_\theta + f(Y_\theta), \label{eq:res_y}\\
...
\mathcal{R}_u(x,t;\psi,\phi) &:= \alpha U_\psi - \partial_n \Lambda_\phi, \quad (x,t) \in \Sigma. \label{eq:res_u}
\end{align}

Because the boundary and initial/terminal conditions are enforced by construction (hard constraints) rather than penalized, the only residuals that remain are the PDE residuals \eqref{eq:res_y}--\eqref{eq:res_u} together with the (typically small, if the hard constraint for the initial condition is not also imposed exactly) initial/terminal mismatches. The indirect PINN loss, matching \eqref{eq:indirect_loss}, is

\begin{equation}
\label{eq:loss_indirect}
\mathcal{L}_{\text{ind}}(\theta,\psi,\phi) = w_y \mathcal{L}_y + w_\lambda \mathcal{L}_\lambda + w_u \mathcal{L}_u + w_{\text{ic}} \mathcal{L}_{\text{ic}} + w_T \mathcal{L}_T,
\end{equation}
...
\mathcal{L}_T &:= \frac{1}{N_T} \sum_{i=1}^{N_T} |\Lambda_\phi(x_i,T)|^2.
\end{align}
...
\item[(A2)] \textbf{Regularity of the Dirichlet control problem.} Dirichlet boundary control of a parabolic equation is, in general, a low-regularity problem: for boundary data $u\in L^2(\Sigma)$ the state $y$ is typically only a \emph{very weak solution} (transposition solution), and the trace $\partial_n\lambda|_\Sigma$ appearing in \eqref{eq:stat_res} need not exist in the classical sense (see e.g.\ Lasiecka \& Triggiani; Tr\"oltzsch, \emph{Optimal Control of Partial Differential Equations}, Ch.\,5). We assume throughout that the data $(y_0, y_d, u^*)$ are smooth enough, and $\Omega$ is regular enough, that $(y^*,\lambda^*)$ lie in $H^{2,1}(Q)$-type spaces in which all traces below are classically well defined. This is a genuine restriction, not a technicality; results that require only $u \in L^2(\Sigma)$ are outside the scope of the estimates below.
...
\;+\; \eta_{\text{ic}}^2 + \eta_T^2,
\end{equation}

where $\eta_{\text{ic}}^2$ and $\eta_T^2$ are the analogous quadrature approximations of $\|Y_\theta(\cdot,0)-y_0\|_{L^2(\Omega)}^2$ and $\|\Lambda_\phi(\cdot,T)\|_{L^2(\Omega)}^2$. By construction $\eta^2$ is a quadrature estimate of the continuous quantity $\|\mathcal{R}_y\|_{L^2(Q)}^2+\|\mathcal{R}_\lambda\|_{L^2(Q)}^2+\|\mathcal{R}_u\|_{L^2(\Sigma)}^2+\|Y_\theta(\cdot,0)-y_0\|^2+\|\Lambda_\phi(\cdot,T)\|^2$ that enters Theorem~\ref{thm:apriori}, evaluated on a point set independent of the one used for training. If desired, a \emph{local} (space-time cell) version can still be reported for visualization, $\eta^2 = \sum_{E} \eta^2(E)$ with $E$ ranging over cells of a fixed post-hoc partition of $\overline{Q}$ used only for plotting, but we stress that this partition plays no role in the estimator's derivation and no jump terms are included.
...
\begin{equation}
\| (e_y, e_\lambda, e_u) \|_{\mathcal{E}} \;\lesssim\; \eta + \mathcal{E}_{\text{quad}},
\end{equation}
...
\begin{proof}[Sketch of proof]
Immediate from Theorem~\ref{thm:apriori} once one identifies $\eta$ as a quadrature approximation of $\epsilon_{\text{loss}}$: apply \eqref{eq:apriori_estimate} with $\epsilon_{\text{loss}}$ replaced by $\eta + \mathcal{E}_{\text{quad}}$ and $\epsilon_{\text{net}}$ absorbed into the implicit constant (which is legitimate once the network has been trained to near-zero loss, so that $\epsilon_{\text{net}}$ is itself of the same order as $\eta$).
\end{proof}
